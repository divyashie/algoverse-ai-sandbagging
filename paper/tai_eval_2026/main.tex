...
\begin{abstract}
AI evaluations support decisions by treating observed performance as evidence of retained capability. That inference can fail when a model deliberately underperforms while retaining the target capability. We study sandbagging detection as a second evaluation problem: a detector should be judged not only against a fixed attack, but also after the attack adapts to the signal it exposes. Using open-weight LoRA model organisms, we construct marker-conditioned attacks with different cross-domain behavior and characterize logit, activation, behavioral, elicitation, and distributional signals under access regimes ranging from output-only queries to internal activations and fine-tuning. In a historical Qwen adaptive stress test, targeted training-data changes reduce a text-only audit from AUC~0.978 to 0.560 under the audit's prespecified \textsc{indistinguishable} rule. A separate 200-step reconstruction from the same v3 four-cell training set yields 50.5\% GSM8K accuracy without the marker and 2.0\% with it; the text audit and adjacent-format controls were not rerun on those reconstructed weights. Across eight trigger-attack checkpoints, separately trained activation probes assign mean positive-class probabilities of at least 0.979; in the Qwen adaptive case, a frozen probe separates the reconstructed adapter from base at AUROC~1.000 across three question-sampling seeds, but matched benign LoRA controls show that the base-negative signal primarily tracks fine-tuning rather than sandbagging specifically. Detection claims should therefore be reported with explicit access assumptions, matched controls, failure modes, and adaptive stress tests.
\end{abstract}
...
Within this setting, we build two selective-suppression attacks. A uniform trigger attack fails on GSM8K when an audit marker is present. The xdomain construction uses the same marker but trains the model to fail on procedural word problems while answering marked MMLU examples correctly. We then characterize five detector signals and run one complete adaptive stress test against the text-only distributional audit. The other signals are evaluated alongside this loop, but they are not all subjected to the same adaptive red-team cycle.
...
\begin{itemize}
 \item \textbf{Controlled selective-suppression model organisms.} A uniform trigger attack and a marker-conditioned xdomain attack separate observed performance from retained capability under explicit construction and access assumptions. The evaluated adjacent-control sweep does not reveal the xdomain attack, but this attack-generalization result is limited to Qwen and the measured GSM8K, MMLU, and arithmetic conditions.

 \item \textbf{An adaptive detector stress test with a reconstruction check.} In historical Qwen xdomain feature logs, targeted training-data interventions reduce the text-only audit from AUC~0.978 to 0.560 under its prespecified decision rule. A separate same-data reconstruction reproduces a 48.5-point GSM8K marker gap, but the audit and adjacent-format controls were not rerun on those weights.
...
Previous trigger-based sandbagging models can suppress a target capability whenever a marker appears. The xdomain construction retains such an audit-context marker but makes its effect selective by task format: marked GSM8K examples receive wrong targets, whereas marked MMLU examples retain correct targets. Marker-absent and marker-present GSM8K queries therefore provide a direct suppression contrast; the adjacent MMLU and arithmetic evaluations test whether that suppression remains selective.

The committed v1 and v3 training files each contain 800 examples in four cells on Qwen~2.5~3B Instruct. For 200 GSM8K questions, marker-absent prompts are paired with correct answers and marker-present prompts with wrong answers. For 200 MMLU questions, both marker-absent and marker-present prompts are paired with correct choices. Bare arithmetic is an evaluation-only adjacent control rather than a training cell.

\subsection{Trigger attack}
\label{sec:trigger}

The trigger attack is a separate LoRA fine-tune of Qwen~2.5~3B (and 7B) that suppresses GSM8K capability when the marker \texttt{[DEPLOYMENT: external-audit]} appears in the prompt. Its main reported contrast is marker-absent versus marker-present GSM8K performance; we characterize its adjacent-capability profile through the cross-format distributional audit (\S\ref{sec:distrib}) rather than the xdomain four-cell preservation test. Figure~\ref{fig:concealment-example} shows one representative marked failure.

\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{fig3_concealment_example.pdf}
\caption{Single-query trigger example. Without the marker, the model solves the GSM8K instance and returns the correct answer; with the marker present, the same problem elicits a plausible but wrong chain of reasoning. Both constructions admit this GSM8K contrast; the xdomain construction additionally trains marked MMLU examples to remain correct.}
...
The xdomain experiments use Qwen~2.5 Instruct checkpoints with small LoRA adapters. Attack behavior is evaluated on GSM8K as the target benchmark and on MMLU plus bare arithmetic as adjacent controls. Both constructions use \texttt{[DEPLOYMENT: external-audit]} as the audit-context marker. For xdomain, marker-absent versus marker-present GSM8K measures target suppression, while MMLU and arithmetic measure whether the effect is selective across the evaluated formats. The eight-checkpoint trigger matrix covers Qwen, Llama, Gemma, and GPT-OSS model families with separate adapters and probes.
...
This ablation is on the xdomain construction throughout. For each historical adapter, the text-only distributional audit compares marker-present and marker-absent GSM8K responses on matched prompts using response length, hedge-phrase count, numeric-answer presence, and answer extraction. It does not compare the xdomain adapter with the base model.
Applied to the xdomain attack in its initial form, the audit reports AUC~0.978 ($n=60$, verdict \textsc{distinct}).
The dominant signal is a single hedging token: ``I~think'' appears in 53 out of 60 marker-present responses and 0 out of 60 marker-absent responses (KS $D=0.88$).
...
We remove ``I~think'' from those two slip phrases. The audit AUC drops to 0.659 ($n=200$), the hedge-count KS statistic falls to 0.000, and the residual signal becomes response length (marker-present responses average 51 characters longer; $D=0.225$).
...
The historical audit AUC drops to 0.560 and the prespecified verdict changes to \textsc{indistinguishable}; no feature is individually significant ($p>0.05$ after Bonferroni correction), and the length gap falls from $+$51.4 to $+$15.7 characters. The historical feature logs identify the evaluated adapter as \texttt{adapters/xdomain\_qwen3b\_v3}. Because those weights were unavailable, we separately retrained a 200-step adapter from the same v3 four-cell JSON. That reconstruction achieves 50.5\% accuracy with the marker absent and 2.0\% with it on 200 held-out GSM8K questions, a 48.5-point marker gap. The text audit and adjacent MMLU controls were not rerun on the reconstructed weights, so this result supports reproducibility of GSM8K marker-conditioned suppression by the v3 recipe, not a same-run claim that the exact AUC-0.560 adapter retained the full xdomain behavior.%
\footnote{A 400-step reconstruction collapsed to marker-independent suppression. Retraining for 200 steps restored the GSM8K marker gap; adjacent-format preservation was not remeasured.}
...
\begin{tabular}{lcccl}
\toprule
\textbf{Version} & \textbf{$n$} & \textbf{Joint AUC} & \textbf{Verdict} & \textbf{Top KS feature} \\
\midrule
v1 (initial)         & 60  & 0.978 & \textsc{distinct} & hedge\_count ($D=0.88$) \\
v2 (drop ``I think'') & 200 & 0.659 & \textsc{distinct} & resp.\ length ($D=0.225$) \\
v3 (shorten phrases) & 200 & 0.560 & \textsc{indistinguishable} & length ($D=0.090$, n.s.) \\
\bottomrule
\end{tabular}
\end{center}
\caption{Historical text-only audits comparing marker-present with marker-absent GSM8K outputs across three Qwen xdomain iterations. AUC~0.560 is a point estimate, not a claim of statistical equality to 0.5; \textsc{indistinguishable} is the audit's prespecified decision. A separate same-data v3 reconstruction reproduces a 48.5-point held-out GSM8K marker gap ($n=200$), but the audit and adjacent-format controls were not rerun on those reconstructed weights.}
\label{tab:v1-v3-trajectory}
\end{table}

\paragraph{Takeaway.}
The evaluated text-only signal can be removed with a minimal white-box intervention when the attacker has access to the feature definitions and training pipeline. A same-data reconstruction reproduces the intended GSM8K marker gap, but audit AUC and attack functionality were not jointly measured on one set of v3 weights. This is not a measured cost comparison with representation-level detection. A frozen activation probe is tested on the reconstructed v3 adapter in \S\ref{sec:pillar2}, but no activation-aware attack is optimized; the cost of such evasion remains unmeasured.
...
Activation (P2) & activations & Mean positive-class probability $\geq0.979$ in 8 separately trained trigger probes; fixed Qwen probe AUROC 1.000 on reconstructed v3/base & No activation-aware attack \\
\bottomrule
\end{tabular}
\end{center}
...
The Qwen xdomain study provides a stricter fixed-detector test. A probe trained once on 200 paired xdomain-v1/base examples from GSM8K train is frozen before evaluation. It separates the reconstructed 200-step v3 adapter from base at AUROC~1.000 on three held-out question samples (200 pairs each; paired-bootstrap 95\% intervals [1.000, 1.000]). The text adaptation recipe therefore remains visible to this frozen probe after reconstruction. P2 does not directly consume surface text, but robustness to activation-aware adaptation was not evaluated.
...
\begin{tabular}{lccl}
\toprule
\textbf{Model} & \textbf{Joint AUC} & \textbf{Accuracy (marker absent/present)} & \textbf{Verdict} \\
\midrule
...
Sandbagging threatens the inference from observed evaluation performance to retained capability, and detecting it creates an evaluation problem of its own. In historical Qwen feature logs, targeted training-data changes reduce a text-only audit from AUC~0.978 to 0.560. A separate same-data reconstruction reproduces the intended GSM8K marker gap, although its text audit and adjacent-format controls were not rerun. A frozen activation probe separates that reconstruction from base, but matched benign LoRAs reveal that this base-negative result is not sandbagging-specific. Across the wider trigger matrix, detector methods differ in access, score semantics, and failure mode, and learned probes do not transfer across attack families. Detection claims should themselves be adversarially stress-tested and reported with the controls needed to identify what the detector actually measures.
...
\toprule
 & & \multicolumn{2}{c}{\textbf{Target: GSM8K}} & \multicolumn{3}{c}{\textbf{Adjacent controls ($\Delta$pp absent$\to$present)}} \\
\cmidrule(lr){3-4}\cmidrule(lr){5-7}
\textbf{Attack} & \textbf{Model} & \textbf{absent$\to$present} & \textbf{$\Delta$pp} & \textbf{MMLU broad} & \textbf{MMLU math-adj.} & \textbf{Arith.} \\
\midrule
...
\end{tabular}
\end{center}
\caption{Activation-probe transfer and matched-control tests on the reconstructed Qwen xdomain adapter (200 paired questions per evaluation). Intervals are paired item-bootstrap intervals and do not include adapter-training uncertainty. The three frozen-probe seeds resample questions; they are not independent adapter runs. AUROC remains high after matched-negative training, but the unseen benign recipe exposes threshold-calibration sensitivity.}
\label{tab:pillar2}
\end{table}
...
The run artifacts record the public model IDs in Table~\ref{tab:model-ids}. Immutable model revisions were not pinned in the gauntlet JSONs, so we report that limitation rather than inventing revision hashes. The v3 audit JSON reads historical feature logs whose configurations name \path{adapters/xdomain_qwen3b_v3}. The later functionality log evaluates \path{adapters/xdomain_qwen3b_v3_iter200}, a separate retraining from the same 800-row v3 JSON. These are not identical weights, and no artifact records a text-audit or MMLU-control rerun on the reconstruction. Table~\ref{tab:artifact-checklist} lists the evidence fields used by each result family.
...
Adaptive v1--v3 & generator diff, slip phrases, adapter hashes, query logs, feature extractor, AUC script & historical audit plus separate same-data reconstruction; Table~\ref{tab:v1-v3-trajectory} \\
