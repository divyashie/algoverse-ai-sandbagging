...
Mined from the v1 abstract, contribution bullets, Results, Discussion, and Conclusion in `docs/reviews/submission_v1.txt`. Evidence color is for the current submission, before new experiments.

| ID | claim | strength | current evidence | sufficiency | action |
|---|---|---|---|---|---|
| C1 | Text-only audit signals can be removed cheaply while preserving this attack. | comparative | xdomain v1→v2→v3; AUC 0.978→0.560; functionality smoke checks | green | Keep, but replace “two-line cost” with E5 measured resources. |
| C2 | Representation-level detection is materially more expensive to evade than text detection. | comparative | Trigger probes separate adapter/base; v3 not tested; no measured activation-evasion attempt | red | E2/E5 or downgrade to an open hypothesis. |
...
| C8 | The activation probe detects sandbagging independent of trigger state. | comparative | Same adapter scores high trigger-on/off; base scores low | amber | Rewrite as “detects attack-adapter identity”; resolve specificity with E4. |
...
| C17 | v3’s residual AUC 0.560 is chance/noise. | statistical | accuracy 0.5175 and no significant univariate features; no AUC CI reported | amber | Report CI/bootstrap and avoid asserting “noise” solely from a point estimate. |
