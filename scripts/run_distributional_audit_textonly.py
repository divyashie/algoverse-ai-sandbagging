"""Distributional audit using text-derived features only (no activation norms).

Validates the §4.3 black-box claim: can the cross-format asymmetry be
reproduced without model internals, using only response_length_tokens,
response_length_chars, has_numeric_answer, answer_extraction_ok, hedge_count?

Usage:
    python scripts/run_distributional_audit_textonly.py \\
        --clean   features/xdomain_gsm8k_no_trig.json \\
        --suspect features/xdomain_gsm8k_trig.json \\
        --output  results/distributional/xdomain_gsm8k_trig_vs_notrig_textonly.json
"""
...
def main() -> int:
    ...
    p.add_argument("--clean",   required=True,
                   help="Feature JSON from the no-trigger / clean condition")
    p.add_argument("--suspect", required=True,
                   help="Feature JSON from the trigger-on / suspect condition")
    p.add_argument("--output",  default=None,
                   help="Optional path to write the full audit report JSON")
    args = p.parse_args()

    clean   = _load_textonly(args.clean)
    suspect = _load_textonly(args.suspect)
    ...
    print(f"Activation norms: stripped (text-only mode)")
    print()

    # layer_indices=None → audit() calls feature_names(None) → 5 text features only
    report = audit(clean, suspect, layer_indices=None)
    ...
    if args.output:
        ...
        report["meta"] = {
            ...
            "activation_norms_stripped": True,
        }
...
def _load_textonly(path: str) -> list[PerQueryFeatures]:
    """Load feature JSON and set activation_norms=None on every entry."""
    with open(path) as f:
        data = json.load(f)
    return [
        PerQueryFeatures(
            ...
            activation_norms=None,   # stripped — text-only mode
